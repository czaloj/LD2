package  
{
	...
	public class SlimeAugmentor extends FlxSprite 
	{
		...
		public var hp:int = EntityStats.AUGMENTOR.health;
		public var stats:EntityStats = EntityStats.AUGMENTOR;
		...
		public function SlimeAugmentor(gameStateRefIn:GameState, X:Number=0, Y:Number=0, SimpleGraphic:Class=null) 
		{
		...
		override public function update():void {
			...
			if (!inAir) {
				...
				acceleration.y = 1000;
				if (dest > x) {
					velocity.x += EntityStats.AUGMENTOR.moveSpeed+(Math.random() * 75);
				}
				if (dest < x) {
					velocity.x -= EntityStats.AUGMENTOR.moveSpeed+(Math.random() * 75);
				}
